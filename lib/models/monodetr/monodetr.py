...
class ExtendedSetCriterion(SetCriterion):
    ...
    def cbr_loss_3D(self, outputs:dict, targets:dict, indices, num_boxes, targets_info)->dict:
        ...
        # u,v loss
        # NOTE: This commented code hereunder is an alternative way to get img_size, but I decided to get it directly 
        # from the targets_info dict, which I adapted in the kitti_dataset class
        # targets_img_size = torch.zeros_like(target_3dcenter)
        # for b, idxs in enumerate(indices): # each batch has a tuple of 2 tensors, batch and target
        #     for i in range(int(num_boxes)): # only relevant length of the target idx tensor
        #         targets_img_size[i] = targets_info['img_size'][b]
